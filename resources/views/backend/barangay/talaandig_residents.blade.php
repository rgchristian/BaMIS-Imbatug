...
<nav class="page-breadcrumb">
        ...
        <ol class="breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Talaandig Residents</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> Talaandig residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($talaandigResidents as $key => $barangay_talaandig_residents)
                   
    @php
        $hasCase = $barangay_talaandig_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_talaandig_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_talaandig_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_talaandig_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_talaandig_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_talaandig_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_talaandig_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_talaandig_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_talaandig_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.talaandig.resident', $barangay_talaandig_residents->id) }}">
        ...
        <a href="{{ route('edit.talaandig.resident', $barangay_talaandig_residents->id) }}">
        ...
        <a href="{{ route('delete.talaandig.resident', $barangay_talaandig_residents->id) }}" id="delete"> 
