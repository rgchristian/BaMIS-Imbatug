...
<nav class="page-breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Solo Parent Residents</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> solo parent residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($widowResidents as $key => $barangay_widow_residents)
                   
    @php
        $hasCase = $barangay_widow_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_widow_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_widow_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_widow_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_widow_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_widow_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_widow_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_widow_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_widow_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.soloparent.resident', $barangay_widow_residents->id) }}">
        ...
        <a href="{{ route('edit.soloparent.resident', $barangay_widow_residents->id) }}">
        ...
        <a href="{{ route('delete.soloparent.resident', $barangay_widow_residents->id) }}" id="delete"> 
