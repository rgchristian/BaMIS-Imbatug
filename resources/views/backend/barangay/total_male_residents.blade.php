...
<nav class="page-breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Male Residents</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> male residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($maleResidents as $key => $barangay_male_residents)
                   
    @php
        $hasCase = $barangay_male_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_male_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_male_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_male_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_male_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_male_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_male_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_male_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_male_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.male.resident', $barangay_male_residents->id) }}">
        ...
        <a href="{{ route('edit.male.resident', $barangay_male_residents->id) }}">
        ...
        <a href="{{ route('delete.male.resident', $barangay_male_residents->id) }}" id="delete"> 
