...
<nav class="page-breadcrumb">
        ...
        <ol class="breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Purok 3</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> Purok 3 residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($purokThreeResidents as $key => $barangay_purok_three_residents)
                   
    @php
        $hasCase = $barangay_purok_three_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_purok_three_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_purok_three_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_purok_three_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_purok_three_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_purok_three_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_purok_three_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_purok_three_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_purok_three_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.purok.three.resident', $barangay_purok_three_residents->id) }}">
        ...
        <a href="{{ route('edit.purok.three.resident', $barangay_purok_three_residents->id) }}">
        ...
        <a href="{{ route('delete.purok.three.resident', $barangay_purok_three_residents->id) }}" id="delete"> 
