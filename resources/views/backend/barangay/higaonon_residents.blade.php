...
<nav class="page-breadcrumb">
        ...
        <ol class="breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Higaonon Residents</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> Higaonon residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($higaononResidents as $key => $barangay_higaonon_residents)
                   
    @php
        $hasCase = $barangay_higaonon_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_higaonon_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_higaonon_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_higaonon_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_higaonon_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_higaonon_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_higaonon_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_higaonon_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_higaonon_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.higaonon.resident', $barangay_higaonon_residents->id) }}">
        ...
        <a href="{{ route('edit.higaonon.resident', $barangay_higaonon_residents->id) }}">
        ...
        <a href="{{ route('delete.higaonon.resident', $barangay_higaonon_residents->id) }}" id="delete"> 
