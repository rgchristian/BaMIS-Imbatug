...
<nav class="page-breadcrumb">
        ...
        <ol class="breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Manobo Residents</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> Manobo residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($manoboResidents as $key => $barangay_manobo_residents)
                   
    @php
        $hasCase = $barangay_manobo_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_manobo_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_manobo_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_manobo_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_manobo_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_manobo_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_manobo_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_manobo_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_manobo_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.manobo.resident', $barangay_manobo_residents->id) }}">
        ...
        <a href="{{ route('edit.manobo.resident', $barangay_manobo_residents->id) }}">
        ...
        <a href="{{ route('delete.manobo.resident', $barangay_manobo_residents->id) }}" id="delete"> 
