...
<nav class="page-breadcrumb">
					<ol class="breadcrumb">
            <a href="{{ route('barangay.residents') }}" class="btn btn-inverse-primary" data-bs-toggle="tooltip" data-bs-placement="top" title="Back">Back</a>
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Barangay Residents Archive</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents archive</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($archive_residents as $key => $archive_res)
                   
    @php
        $hasCase = $archive_res->moral === 'Has Unsettled Case';
        $inactive = $archive_res->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($archive_res->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($archive_res->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $archive_res->name }}</td>
    <td style="text-align: center;">
      <img class="rounded-circle resident-image" 
          src="{{ (!empty($archive_res->photo)) ? asset($archive_res->photo) : url('upload/no_image.png') }}" 
          alt="{{ (!empty($archive_res->photo)) ? '' : 'default_profile' }}">
    </td>
    <td style="text-align: center;">
      @if(!empty($archive_res->birthdate))
          {{ date('Y-m-d', strtotime($archive_res->birthdate)) }}
      @endif
    </td>
    <td style="text-align: center;">
    @if(!empty($archive_res->age))
        {{ $archive_res->age }}
    @endif
    </td>
    <td style="text-align: center;">{{ $archive_res->sex }}</td>
    <td>
      <div style="text-align: center;">
      <a href="{{ route('restore.resident', $archive_res->id) }}" id="restore">
          <button type="button" class="btn btn-inverse-info btn-icon btn-xs" data-bs-toggle="tooltip" data-bs-placement="top" title="Restore">
            <i data-feather="rotate-ccw"></i>
          </button>
        </a>
        <a href="{{ route('delete.permanent', $archive_res->id) }}" id="delete">
