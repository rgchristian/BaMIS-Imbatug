...
<nav class="page-breadcrumb">
        ...
        <ol class="breadcrumb">
            ...
            <div class="card">
              <div class="card-body">
                <h6 class="card-title">Matigsalug Residents</h6>
                <!-- <p class="text-muted mb-3">Barangay Imbatug<a href="#"> residents</a>.</p> -->
                <p class="text-muted mb-3">Barangay Imbatug<a href="#"> Matigsalug residents</a>.</p>
                ...
                <div class="table-responsive">
                  <table id="dataTableExample" class="table border-secondary border-top table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach($matigsalugResidents as $key => $barangay_matigsalug_residents)
                   
    @php
        $hasCase = $barangay_matigsalug_residents->moral === 'Has Unsettled Case';
        $inactive = $barangay_matigsalug_residents->active_participation === 'Inactive';
        $rowClass = '';
  ...
  <tr class="{{ $rowClass }}">
    ...
    @if ($barangay_matigsalug_residents->household_representative == 'Yes')
        {!! DNS2D::getBarcodeSVG($barangay_matigsalug_residents->qr_code, 'QRCODE', 1.7, 1.7, 'white') !!}
    @else
        
    @endif
</td>
    <td style="text-align: center;">{{ $barangay_matigsalug_residents->name }}</td>
    <td style="text-align: center;"><img class="rounded-circle resident-image" src="{{ asset($barangay_matigsalug_residents->photo) }}" alt="profile"></td>
    <td style="text-align: center;">{{ date('Y-m-d', strtotime($barangay_matigsalug_residents->birthdate)) }}</td>
    <td style="text-align: center;">{{ $barangay_matigsalug_residents->age }}</td>
    <td style="text-align: center;">{{ $barangay_matigsalug_residents->sex }}</td>
    <td>
      <div style="text-align: center;">
        <a href="{{ route('view.matigsalug.resident', $barangay_matigsalug_residents->id) }}">
        ...
        <a href="{{ route('edit.matigsalug.resident', $barangay_matigsalug_residents->id) }}">
        ...
        <a href="{{ route('delete.matigsalug.resident', $barangay_matigsalug_residents->id) }}" id="delete"> 
